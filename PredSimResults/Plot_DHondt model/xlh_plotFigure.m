...
% plot_results(subject, 'MS_back')
% plot_results(subject, 'MF_back')
...
function [] = plot_results(model_subject, assistance_pattern_str)
    ...
    for i=1:length(result_paths)
        ...
        if exist(result_paths{i}, 'file') == 2  % Check if the file exists
        ...
        figure(2)
            ...
            if (i>2)
                ...
                xlabel('gait cycle (%)');
                ylabel('assistive torque (Nm) add is positive');
                ...
                subplot(313)
                hold on
                hip_back_assistance_i = R.exo.Texo(:,3);
                plot(hip_back_assistance_i, 'DisplayName',legendName,'LineWidth',2)
    
                title('back assistance')
                xlabel('gait cycle (%)');
                ylabel('assistive torque (Nm) bend to right is positive');
            end
