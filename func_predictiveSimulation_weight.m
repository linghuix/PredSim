...
function [] = func_predictiveSimulation_weight(weight_input)
    ...
    S.subject.save_folder  = fullfile(pathRepo,'PredSimResults',[S.subject.name '_normal' S.weights.type]);
    S.subject.save_folder = fullfile(S.subject.save_folder, [ S.weights.string 'weight'] );
...
%                                     };
    ...
    S.weights.E             = weight_input(1);
    S.weights.q_dotdot      = weight_input(2);
    S.weights.e_arm         = weight_input(3);
    S.weights.pass_torq     = weight_input(4);
    S.weights.a             = weight_input(5);
    S.weights.slack_ctrl    = 1e-6;

    % normalize the weight
    S.weights.normalization = true;

    % NadirPoint and E_IdealPoint on metabolic energy rate
    S.subject.NadirPoint_E = 31.6; 
    S.subject.IdealPoint_E = 0.115; 
    % NadirPoint and E_IdealPoint on joint accelerations
    S.subject.NadirPoint_q_dotdot = 9.42e-2; 
    S.subject.IdealPoint_q_dotdot = 4.16e-4; 
    % NadirPoint and E_IdealPoint on arm excitations
    S.subject.NadirPoint_e_arm = 2.53e-2; 
    S.subject.IdealPoint_e_arm = 4.79e-7; 
    % NadirPoint and E_IdealPoint on passive torques
    S.subject.NadirPoint_pass_torq = 11.2;
    S.subject.IdealPoint_pass_torq = 1.29e-6; 
    % NadirPoint and E_IdealPoint on muscle activations
    S.subject.NadirPoint_a = 0.286;
    S.subject.IdealPoint_a = 5.1e-3; 
