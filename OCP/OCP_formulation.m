...
for j=1:d
    ...
    elseif strcmp(S.metabolicE.model,'Umberger2010')
        % Get metabolic energy rate Umberger et al. (2010)
        [e_totj,~,~,~,~] = f_casadi.getMetabolicEnergySmooth2010all(...
            akj(:,j+1),akj(:,j+1),lMtildej,vMj,Fcej,...
            MuscleMass',pctsts,Fisoj,model_info.mass,S.metabolicE.tanh_b); 
    elseif strcmp(S.metabolicE.model,'Uchida2016')
        % Get metabolic energy rate Uchida et al. (2016)
...
for k=1:N
    for j=1:d
       ...
       % Bhargava et al. (2004)
        ...
        % Umberger et al. (2010)
        elseif strcmp(S.metabolicE.model,'Umberger2010')
            [e_tot_all,~,~,~,~] = f_casadi.getMetabolicEnergySmooth2010all(...
            a_col_opt_unsc(count,:)',a_col_opt_unsc(count,:)',...
        ...
        E_cost2 = E_cost2 + B(j+1)*...
            (f_casadi.J_muscles_exp(e_tot_opt_all,W.E_exp))/model_info.mass*h_opt;
        A_cost2 = A_cost2 + B(j+1)*...
            (f_casadi.J_muscles(a_col_opt(count,:)))*h_opt;      
        Qdotdot_cost2 = Qdotdot_cost2 + B(j+1)*...
            (f_casadi.J_not_arms_dof(qdotdot_col_opt(count,model_info.ExtFunIO.jointi.noarmsi)))*h_opt;
        Pass_cost2 = Pass_cost2 + B(j+1)*...
            (f_casadi.J_lim_torq(Tau_passkj))*h_opt;
        vA_cost2 = vA_cost2 + B(j+1)*...
            (f_casadi.J_muscles(vA_opt(k,:)))*h_opt;
...
E_costf2 = full(E_cost2);
A_costf2 = full(A_cost2);
Arm_costf2 = full(Actu_cost2);
Qdotdot_costf2 = full(Qdotdot_cost2);
Pass_costf2 = full(Pass_cost2);
vA_costf2 = full(vA_cost2);
